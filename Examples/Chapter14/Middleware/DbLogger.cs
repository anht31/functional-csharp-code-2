...
public class DbLogger
{
    ...
    Middleware<SqlConnection, int> ConnectInt;
    ...
    public void LogTest1(LogMessage message) => (
        from conn in Connect
        select conn.Execute("sp_create_log", message, commandType: CommandType.StoredProcedure)
    ).Run();

    public void Log2(LogMessage message) => (
        Connect.Map(x => x.Execute("sp_create_log", message,
            commandType: CommandType.StoredProcedure))
    ).Run();

    public void Log3(LogMessage message) => (
        from time in Time("InsertLog")
        select time
    ).Run();

    public void Log4(LogMessage message) => (
        Time("InsertLog")
            .Map(x => x)
    ).Run();

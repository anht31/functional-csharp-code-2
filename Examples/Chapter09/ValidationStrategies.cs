...
public static partial class ValidationStrategies
{
    ...
    static readonly Validator<int> Failure2 = _ => Error("Invalid 2");
    static readonly Validator<int> Failure3 = _ => Error("Invalid 3");

    public static void Run()
    {
        var validations = List(Success, Failure, Success, Failure2, Failure3);
        var failFast = FailFast(validations);
        var result = failFast(123);
        WriteLine($"\n{result}");

        //BindCanFilterNone();

        //var harvestErrors = HarvestErrors(validations)(2);
    }

    record Subject(Option<int> Opt);
    ...
    public static void BindCanFilterNone()
    {
        //In struct Option<T>
        ...
        var result = Some(33).AsEnumerable();
        WriteLine(result);

        var statedAges = Population.SelectMany(p => p.Opt.AsEnumerable());
        WriteLine(statedAges);
    }
    ...
    // Overload Bind
    // Bind: (IEnumerable<T>, Func<T, Option<R>>) -> IEnumerable<R>
    // func(t): Option<R>
    // .AsEnumerable(): Option<R> -> R (IEnumerable<Error>)

...
{
    using static Examples.Chapter9.PhoneNumber;
    using static F;
    ...
    class Example_WithValidation
    {
        ...
        {
            var validation = Valid(PhoneNumber.Create);

            var validType = validation.Apply(validNumberType(type));
            var validCountry = validType.Apply(validCountryCode(countryCode));
            var result = validCountry.Apply(validNumber(number));
        ...
        Validation<PhoneNumber> CreateValidPhoneNumberMonadCall
              (string typeStr, string countryStr, string numberStr)
              => validNumberType(typeStr).SelectMany(
                  numnberType => validCountryCode(countryStr),
                  (numberType, countryCode) => (numberType, countryCode)
              ).SelectMany(
                  tupple => validNumber(numberStr),
                  (tuple, number) => PhoneNumber.Create(tuple.numberType, tuple.countryCode, number)
            );
